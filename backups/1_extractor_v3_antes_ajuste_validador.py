    ...
    try:
        with open(ruta_log, 'w', encoding='utf-8') as f:
            ...
            if 'validacion' in info_extraccion:
                ...
                f.write(f"  - Filas válidas: {val.get('filas_validas', 0):,}\n")
                f.write(f"  - Filas con errores: {val.get('filas_con_errores', 0):,}\n")
                f.write(f"  - Registros duplicados: {val.get('registros_duplicados', 0)}\n")

                if val.get('total_filas', 0) > 0:
                    tasa = (val.get('filas_validas', 0) / val['total_filas']) * 100
                    f.write(f"  - Tasa de calidad: {tasa:.2f}%\n")
...
def validar_datos_detallado(df, ruta_archivo_salida, tipo_archivo):
    """
    Valida datos extraídos y genera reporte detallado de errores por fila/columna
    ...
    # Definir columnas críticas según tipo
    if tipo_archivo == 'BM':
        columnas_criticas = ['timestamp', 'answers', 'feedbackType']
    else:  # BV
    ...
    # Validar fila por fila (solo las primeras 1000 para no saturar el log)
    ...
    for idx in range(filas_a_validar):
        ...
        if tipo_archivo == 'BM':
            ...
            if 'timestamp' in df.columns and pd.isna(fila['timestamp']):
                errores_fila.append({
                    'columna': 'timestamp',
                    'error': 'Valor nulo en columna crítica',
                    'valor': None
                })

            # Validar answers (debe contener datos)
            if 'answers' in df.columns:
                valor_answers = fila['answers']
                if pd.isna(valor_answers) or str(valor_answers).strip() == '':
                    errores_fila.append({
                        'columna': 'answers',
                        'error': 'Campo vacío o nulo',
                        'valor': valor_answers
                    })
                elif isinstance(valor_answers, str):
                    # Verificar si es JSON malformado
                    if 'Ã' in valor_answers:
                        errores_fila.append({
                            'columna': 'answers',
                            'error': 'Encoding UTF-8 corrupto detectado',
                            'ejemplo': valor_answers[:50] + '...'
                        })
            ...
            if 'Date Submitted' in df.columns and pd.isna(fila['Date Submitted']):
                errores_fila.append({
                    ...
                    'error': 'Fecha de envío faltante',
                    'valor': None
                })

        # Registrar errores si los hay
        if errores_fila:
            validacion['filas_con_errores'] += 1
            if len(validacion['errores_por_fila']) < 100:  # Guardar solo primeros 100 errores
                validacion['errores_por_fila'].append({
            ...
            for error in errores_fila:
                col = error['columna']
                if col not in validacion['errores_por_columna']:
                    validacion['errores_por_columna'][col] = 0
                validacion['errores_por_columna'][col] += 1
        else:
            validacion['filas_validas'] += 1

    # Detectar duplicados
    if tipo_archivo == 'BM' and 'id' in df.columns:
        validacion['registros_duplicados'] = int(df['id'].duplicated().sum())
    elif tipo_archivo == 'BV' and 'Date Submitted' in df.columns:
        validacion['registros_duplicados'] = int(df['Date Submitted'].duplicated().sum())

    # Generar archivo de validación
    archivo_validacion = ruta_archivo_salida.replace('.xlsx', '.validation')
    try:
        with open(archivo_validacion, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("REPORTE DE VALIDACIÓN - 1_extractor.py\n")
            f.write("="*80 + "\n\n")
            ...
            f.write(f"  Filas válidas: {validacion['filas_validas']:,}\n")
            f.write(f"  Filas con errores: {validacion['filas_con_errores']:,}\n")
            f.write(f"  Registros duplicados: {validacion['registros_duplicados']}\n\n")

            if validacion['columnas_criticas_faltantes']:
                f.write("⚠️  COLUMNAS CRÍTICAS FALTANTES:\n")
                for col in validacion['columnas_criticas_faltantes']:
                    f.write(f"  • {col}\n")
                f.write("\n")
            ...
            if validacion['valores_nulos_por_columna']:
                ...
                for col, info in sorted(validacion['valores_nulos_por_columna'].items(),
                    ...
                    f.write(f"  • {col}: {info['cantidad']:,} ({info['porcentaje']}%)\n")
                f.write("\n")

            if validacion['errores_por_columna']:
                f.write("ERRORES POR COLUMNA:\n")
                for col, count in sorted(validacion['errores_por_columna'].items(),
                                        key=lambda x: x[1], reverse=True):
                    f.write(f"  • {col}: {count} errores\n")
                f.write("\n")

            if validacion['errores_por_fila']:
                f.write("ERRORES DETALLADOS POR FILA (Primeros 100):\n")
                f.write("-"*80 + "\n")
                for error_info in validacion['errores_por_fila'][:100]:
                    f.write(f"\nFila {error_info['fila']} del Excel:\n")
                    for error in error_info['errores']:
                        f.write(f"  • Columna '{error['columna']}': {error['error']}\n")
                        if 'valor' in error and error['valor'] is not None:
                            f.write(f"    Valor: {error['valor']}\n")
                        if 'ejemplo' in error:
                            f.write(f"    Ejemplo: {error['ejemplo']}\n")
                f.write("\n")

            # Calcular tasa de calidad
            if validacion['total_filas'] > 0:
                tasa_calidad = (validacion['filas_validas'] / validacion['total_filas']) * 100
                f.write(f"TASA DE CALIDAD: {tasa_calidad:.2f}%\n")
                if tasa_calidad >= 95:
                    f.write("ESTADO: ✅ EXCELENTE - Datos listos para inserción\n")
                elif tasa_calidad >= 80:
                    f.write("ESTADO: ⚠️  ACEPTABLE - Revisar errores antes de insertar\n")
                else:
                    f.write("ESTADO: ❌ CRÍTICO - Revisar archivo antes de continuar\n")
...
def extraer_datos(ruta_archivo, max_registros=None, directorio_salida=DIRECTORIO_SALIDA):
    ...
    try:
        ...
        if validacion['filas_con_errores'] > 0:
            print(f"   ⚠️  Errores encontrados: {validacion['filas_con_errores']} filas con problemas")
            print(f"   📋 Ver detalles en: {os.path.basename(str(archivo_salida).replace('.xlsx', '.validation'))}")
        else:
            print(f"   ✅ Validación exitosa: Todos los registros son válidos")

        if validacion['registros_duplicados'] > 0:
            print(f"   ⚠️  Duplicados detectados: {validacion['registros_duplicados']} registros")
            info_extraccion['advertencias'].append(f"{validacion['registros_duplicados']} registros duplicados detectados")
